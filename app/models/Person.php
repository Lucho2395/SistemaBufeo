...
class Person{
    ...
    public function validardni($dni_persona){
        try {
            $sql = 'select * from person where person_dni = ?';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$dni_persona]);
            $resultado = $stm->fetch();
            (isset($resultado->id_person)) ? $result=true:$result=false;
        } catch (Exception $e){
            $this->log->insert($e->getMessage(), get_class($this).'|'.__FUNCTION__);
            $result = [];
        }
    ...
    public function validardnieditar($dni_persona, $id_persona){
        try {
            $sql = 'select * from person where person_dni = ? and id_person <> ?';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$dni_persona , $id_persona]);
            $resultado = $stm->fetch();
            (isset($resultado->id_person)) ? $result=true:$result=false;
        } catch (Exception $e){
            $this->log->insert($e->getMessage(), get_class($this).'|'.__FUNCTION__);
